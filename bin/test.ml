...
let parse_tests =
  ...
  let test_parse_int () =
    check expr "same int" (desuggar_expr [] (parse (`Atom "5"))) (Num (5L,1))
  in
...
let interp_tests =
  ...
  let test_interp_num () =
    check value "same int" (interp (Num (42L,1))) (NumV 42L)
  in
...
let interpreter (src : string) : string =
  let open Interp in
  let e = desuggar_expr [] Parse.(parse (sexp_from_string src)) in
